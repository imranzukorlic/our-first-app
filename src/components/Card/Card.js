...
const Card = ({ images, title, price, text }) => {
  return (
    <div className="bg-gray-100 w-full min-h-screen gap-4 flex-wrap flex justify-center items-center">
      <div className="w-60 px-6 bg-white rounded py-6 transform transition-all hover:translate-y-2">
        <img className="h-50 object-cover " src={images} alt="slika" />
        ...
        <span className="flex items-center ml-23  mt-4 ">
          <img src={image} className="h-3 px-1 " alt="slika"></img>
          <img src={image} className="h-3 px-1" alt="slika"></img>
          <img src={image} className="h-3 px-1" alt="slika"></img>
        </span>
        <div className="flex justify-between">
          <button className="bg-green-400 px-4 py-1 h-30 w-30 ease-in duration-200 hover:bg-violet-600 text-white rounded mt-3 ml-1 ">
            Add to cart
          </button>
